...
from jps_backtest import (
    ...
    select_freq_only,
    select_ensemble,
    select_weekday_consensus_exacto,
    select_weekday_inverse_recent,
    select_weekday_recent,
    select_weekday_session_recent,
    select_global_frequency_prior,
    select_weekday_recent30_plus_global_prior,
    select_weekday_session_plus_global_prior,
    select_weekday_recent30_session_global,
)
...
STRATEGY_SELECTORS = {
    ...
    "global_frequency_prior":  select_global_frequency_prior,
}
...
STRATEGY_PROFILE_OVERRIDE = {
    ...
    "weekday_session_recent30": "exacto_only",
    "weekday_recent15":        "exacto_only",
    "weekday_consensus_exacto": "exacto_only",
    "weekday_inverse_recent":  "exacto_only",
    "exacto_signal_gate":      "exacto_only",
    "global_frequency_prior":  "exacto_only",
    "weekday_recent30_plus_global_prior": "exacto_only",
    "weekday_session_plus_global_prior": "exacto_only",
    "weekday_recent30_session_global": "exacto_only",
...
def main():
    ...
    if effective_strategy in STRATEGY_SELECTORS:
    ...
    elif effective_strategy in ("weekday_specific", "weekday_recent30", "weekday_recent15"):
        ...
        if len(wd_draws) < 30:
            ...
            sys.exit(1)
        if effective_strategy == "weekday_recent15":
            wd_draws = wd_draws[-15:]
            print(f"  Sub-corpus    : últimos 15 sorteos de {target_date.strftime('%A')}")
        elif effective_strategy == "weekday_recent30":
    ...
    elif effective_strategy == "weekday_session_recent30":
        target = {"dia": f"{draw_date}T00:00:00", "session": args.session}
        numbers = select_weekday_session_recent(draws, target, args.n)
        print(f"  Sub-corpus    : weekday + sesión {args.session}, últimos 30 con fallback")

    elif effective_strategy == "weekday_consensus_exacto":
        target = {"dia": f"{draw_date}T00:00:00", "session": args.session}
        numbers = select_weekday_consensus_exacto(draws, target, args.n)
        print(f"  Sub-corpus    : consenso Exacto global + weekday + sesión + recent")

    elif effective_strategy == "weekday_inverse_recent":
        target = {"dia": f"{draw_date}T00:00:00", "session": args.session}
        numbers = select_weekday_inverse_recent(draws, target, args.n)
        print(f"  Sub-corpus    : weekday recent30 excluyendo vistos recientes")

    elif effective_strategy == "weekday_recent30_plus_global_prior":
        target = {"dia": f"{draw_date}T00:00:00", "session": args.session}
        numbers = select_weekday_recent30_plus_global_prior(draws, target, args.n)
        print(f"  Sub-corpus    : weekday_recent30 + global_frequency_prior")

    elif effective_strategy == "weekday_session_plus_global_prior":
        target = {"dia": f"{draw_date}T00:00:00", "session": args.session}
        numbers = select_weekday_session_plus_global_prior(draws, target, args.n)
        print(f"  Sub-corpus    : weekday_session_recent30 + global_frequency_prior")

    elif effective_strategy == "weekday_recent30_session_global":
        target = {"dia": f"{draw_date}T00:00:00", "session": args.session}
        numbers = select_weekday_recent30_session_global(draws, target, args.n)
        print(f"  Sub-corpus    : weekday_recent30 + weekday_session_recent30 + global_frequency_prior")
    ...
    elif effective_strategy in ("signal_only_play", "exacto_signal_gate"):
        ranked = report.get("ranked_numbers", [])
        if not ranked:
            print("\n  ERROR: report no tiene ranked_numbers.")
            sys.exit(1)
        top1_z = abs(ranked[0].get("z_score", 0))
        threshold = 2.0 if effective_strategy == "exacto_signal_gate" else 2.5
        print(f"  Top-1 z-score : {top1_z:.2f}  (umbral: {threshold:.1f})")
        if top1_z <= threshold:
            print(f"\n  ⚠ SEÑAL DÉBIL — la estrategia {effective_strategy} recomienda NO APOSTAR esta sesión.")
        ...
        if not weights:
            ...
            sys.exit(1)
        top1 = max(weights.items(), key=lambda x: x[1])[0]
        numbers = [int(top1)]
    ...
    # Top-10 Exacto weights snapshot (para auditoría: si en el futuro queremos revisar
    # qué pesos había en ese momento, no tenemos que recomputar)
    weights = report.get("weights", {})
    top10_weights = dict(sorted(weights.items(), key=lambda x: x[1], reverse=True)[:10])
